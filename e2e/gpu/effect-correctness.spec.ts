...
function guardGpu(): void {
  ...
  console.warn(
    `[gpu] environment cannot run GPU compute: ${preflight.error}`
    + (ALLOW_GPU_SKIP ? ' (ALLOW_GPU_SKIP=1 -> skipping)' : ''),
  );
  test.skip(ALLOW_GPU_SKIP, `environment cannot run GPU compute: ${preflight.error}`);
  throw new Error(`environment cannot run GPU compute (${preflight.kind}): ${preflight.error}`);
}
